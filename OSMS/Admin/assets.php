...
<p class="bg-dark text-white p-2 text-center"> Product/Part Detail</p>
<?php
$sql="SELECT * FROM assert_tb";
...
if($result->num_rows>0){
    ...
    echo '<th scope="col">Orignal Price(per item)</th>';
    echo '<th scope="col">Selling Price(per item)</th>';
    ...
    while($row=$result->fetch_assoc()){
        ...
        echo '<td>';
        echo '<form action="editproduct.php" method="POST" class="d-inline mr-1">';
        ...
        echo '</form>';
        echo '<form action="" method="POST" class="d-inline mr-1">';
        echo '<input type="hidden" name="id" value='.$row['pid'].'>';
        ...
        echo '<input type="hidden" name="id" value='.$row['pid'].'>';
        echo '<button class="btn btn-warning d-inline mr-1" name="issue" value="issue" type="submit"><i class="fas fa-handshake"></i></button>';
        echo '</form>';
        echo '</td>';
        echo '</tr>';
    }
...
{
    $sql="DELETE FROM assert_tb WHERE pid={$_REQUEST['id']}";
